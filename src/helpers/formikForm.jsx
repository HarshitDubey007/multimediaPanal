...
const DynamicForm = ({ fields, submitfunction, initialValues }) => {
    const [switchValue, setSwitchValue] = useState(initialValues && initialValues.switchValue);
        ...
        switch (field.type) {
            ...
            case 'multiSelect':
                ...
                if (initialValues && initialValues.action_name === "UPDATE") {
                    fieldValues = field.multiple === true
                        ? field.options?.filter((v) => initialValues[field.name].includes(v.value))
                ...
                return (
                    ...
                    <>
                        <MultiSelect
                            ...
                            onChange={(event, value) => formik.setFieldValue(field.name, value)}
                        />
            ...
            case 'switch':
                return (
                    ...
                    <ColorSwitch
                        label={field.label}
                        checked={formik.values[field.name]}
                        onChange={() => formik.setFieldValue(field.name, !formik.values[field.name])}
                    />
                );
            default:
                return null;
